...
class SystemManager:
    ...
    def _create_builtin_factors(self):
        ...
        builtin_factors = [
            ...
            },
            {
                ...
                'factor_formula': '(close - close.rolling(14).min()) / (close.rolling(14).max() - close.rolling(14).min() + 1e-10)',
                'description': '14日相对强弱指标(pandas实现)'
            },
            {
    ...
    def _create_demo_models(self):
        ...
        demo_models = [
            {
                'model_id': 'demo_random_forest',
                'model_name': '随机森林演示模型',
                'model_type': 'random_forest',
                'factor_list': ['momentum_5d', 'momentum_20d', 'volatility_20d', 'turnover_rate'],
            ...
            {
                'model_id': 'demo_xgboost',
                'model_name': 'XGBoost演示模型',
                'model_type': 'xgboost',
                'factor_list': ['momentum_5d', 'momentum_20d', 'volatility_20d', 'turnover_rate', 'pe_ratio'],
                'target_type': 'return_5d',
                'model_params': {
                    'n_estimators': 100,
                    'max_depth': 6,
            ...
            {
                'model_id': 'demo_lightgbm',
                'model_name': 'LightGBM演示模型',
                'model_type': 'lightgbm',
                'factor_list': ['momentum_5d', 'momentum_20d', 'volatility_20d', 'turnover_rate', 'pb_ratio'],
                'target_type': 'return_5d',
                'model_params': {
                    'n_estimators': 100,
                    'max_depth': 6,
        ...
        for model_config in demo_models:
            try:
                success = ml_manager.create_model_definition(**model_config)
                if success:
                    created_count += 1
                    print(f"  ✅ {model_config['model_id']}")
                else:
                    print(f"  ⚠️ {model_config['model_id']} (可能已存在)")
            except Exception as e:
                print(f"  ❌ {model_config['model_id']}: {e}")

        print(f"✅ 创建了 {created_count} 个演示模型")

    def start_web_server(self, host='127.0.0.1', port=5001, debug=True):
        """启动Web服务器"""
        print(f"\n启动Web服务器...")
    ...
    def run_demo(self):
        """运行演示"""
        print("\n启动系统演示...")
        print("演示脚本 examples/complete_system_example.py 不存在，跳过演示")
